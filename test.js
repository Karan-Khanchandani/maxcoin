...
MongoClient.connect(dsn, (err, db) => {
    ...
    fetchFromAPI((err, data) => {
        ...
        .then((result)=>{
            console.log(`Successfully inserted ${result.length} documents into mongo`)
    ...
    fetchFromAPI((err, data) => {
        ...
        insertRedis(redisClient, data.bpi, (err, results) => {
            ...
            redisClient.zrange('values', -1, -1, 'withscores',(err, result) => {
                if(err) throw err;
                console.log(`Redis: The one month's max value is ${result[1]} and it was reached on ${result[0]}`);
